...
if (_eng) then {
	_vehicle vehicleChat format ["Stop engine in 10s to start resupply. Cash is the only method of payment accepted. Service will take 30 seconds."];
	sleep 10;
	_eng = isengineon _vehicle;
...
if((player == driver _vehicle) && (!_eng))then {
	...
	if(_type == "O_Heli_Light_02_dynamicLoadout_F") then {_price = 1000;};
	if(_type == "I_Heli_light_03_dynamicLoadout_F") then {_price = 1000;};
	if(_type == "B_Heli_Attack_01_dynamicLoadout_F") then {_price = 1000;};
	if(_type == "O_Heli_Attack_02_dynamicLoadout_F") then {_price = 1000;};
	if(_type == "O_Heli_Attack_02_black_F") then {_price = 1000;};
	if(_type == "I_Plane_Fighter_03_dynamicLoadout_F") then {_price = 1500;};
	if(_type == "B_Plane_CAS_01_dynamicLoadout_F") then {_price = 2500;};
	if(_type == "O_Plane_CAS_02_dynamicLoadout_F") then {_price = 2500;};
	if(_type == "B_Plane_Fighter_01_Stealth_F") then {_price = 1500;};
	if(_type == "B_Plane_Fighter_01_F") then {_price = 2500;};
	if(_type == "O_Plane_Fighter_02_Stealth_F") then {_price = 1500;};
