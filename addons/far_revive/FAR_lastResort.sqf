...
_randomSound = selectRandom ["diehard.ogg"];
...
if !(player getVariable ["performingDuty", false]) then
{
	...
	if !(_availableBombs isEqualTo []) then
	{
		...
		if (["Die hard with a vengeance?", "", "Yes", "No"] call BIS_fnc_guiMessage) then
		{
			player setVariable ["performingDuty", true];

			player removeMagazine _magType;
			playSound3D [call currMissionDir + "client\sounds\" + _randomSound, player, false, getPosASL player, 1, 1, 1000];
	...
	{
		titleText ["No boom boom cowboy! Next time bring some explosives!", "PLAIN", 0.5];
	};
};