...
_checks =
{
	...
	switch (true) do
	{
		...
		case (alive _object || {alive _x} count crew _object > 0): { _text = "Action failed! You are not allowed to bury this body" };
		case (doCancelAction): { doCancelAction = false; _text = "Burial cancelled" };
		default
		{
			_failed = false;
			_text = format ["Burying Body %1%2 complete", floor (_progress * 100), "%"];
		};
	};
...
if (_success) then
{
	...
	[format ["You have received a $%1 reward for burying this body", [_money] call fn_numbersText], 5] call mf_notify_client;
};
