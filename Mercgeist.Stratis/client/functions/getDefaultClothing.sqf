...
switch (_side) do
{
	case BLUFOR:
	{
		...
		if (_item == "headgear") then { _result = "H_Cap_headphones" };
	};
	case OPFOR:
	{
		switch (true) do
		{
			...
			default
			{
				if (_item == "uniform") then { _result = "U_O_CombatUniform_oucamo" };
				if (_item == "vest") then { _result = _defaultVest };
			};
		};
	...
	default
	{
		...
		if (_item == "headgear") then { _result = "H_Booniehat_khk_hs" };
	};
};
