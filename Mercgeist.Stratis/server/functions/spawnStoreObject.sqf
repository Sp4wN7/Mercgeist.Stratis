...
private ["_isGenStore", "_isGunStore", "_isVehStore", "_isArmsDealer", "_timeoutKey", "_objectID", "_playerSide", "_objectsArray", "_results", "_itemEntry", "_itemPrice", "_safePos", "_object"];
...
_isArmsDealer = ["ArmsDealer", _npcName] call fn_startsWith;

private _storeNPC = missionNamespace getVariable [_npcName, objNull];
private _marker = _npcName;

if (_key != "" && _player isKindOf "Man" && {_isGenStore || _isGunStore || _isVehStore || _isArmsDealer}) then
{
	...
	if (_isGenStore || _isGunStore || _isArmsDealer) then
	{
		...
		switch (true) do
		{
			...
			case _isArmsDealer: { _objectsArray = staticGunsArray };
		};
