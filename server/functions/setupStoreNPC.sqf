...
if (hasInterface) then
{
	...
	switch (true) do
	{
		...
		case (["ArmsDealer", _npcName] call _startsWith):
		{
			_npc addAction ["<img image='client\icons\store.paa'/> Open Arms Dealer", "client\systems\gunStore\loadgunStore.sqf", [], 1, true, true, "", STORE_ACTION_CONDITION];
		};
